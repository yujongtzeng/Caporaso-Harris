    ...
    private BigInteger N(int d, int r, byte[] alpha, byte[] beta) {  
        ...
        for (int k = 0; k < maxLength; k++) { // the first term
            if (beta[k] > 0) {
                ...
                tempAlpha[k] = (byte) (alpha[k] + 1);  //alpha_+e_k, beta-e_k
                tempBeta[k] = (byte) (beta[k] - 1);    
                ArrayList<Byte> key = Key.make(r, tempAlpha, tempBeta);
                if (curSave.containsKey(key)) {
                    //if (curSave.get(key) == -1) return -1;
                    ans = ans.add(curSave.get(key).multiply(BigInteger.valueOf(k + 1)));
                }
                else if (curDump.containsKey(key)) {
                    //if (curDump.get(key) == -1) return -1;
                    ans = ans.add(curDump.get(key).multiply(BigInteger.valueOf(k + 1)));
                }
                else {
            ...
            for (byte[] bP : parArr.get(j)) {
                for (byte[] aP : parArr.get(d - 1 - j)) {
                    if (arrOP.greater(alpha, aP) && arrOP.greater(bP, beta)) {
                        ...
                        if (rP >= 0 && rP <= maxNode) { // no need to check rP <= maxNode                 
                            ...
                            if (prevMap.containsKey(key)) {
                                ...
                                ans = ans.add(coeff.multiply(prevMap.get(key)));
                            }
